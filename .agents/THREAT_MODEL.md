...
| B10b | Stripe webhook verification is ASYNC (`constructEventAsync`); the sync call is rejected by the Deno SDK and turned every delivery into a 400 on 2026-09-08. Enrolment also runs from `confirm-subscription` (caller must be the athlete or their coach, `requireClientAccess`; reads only that athlete's `client_subscriptions` rows; every write is what the paid Stripe subscription implies, idempotent). | `stripe-webhook`, `confirm-subscription`, `_shared/enrollment.ts`; `tests/edgePatterns.test.ts` | `supabase/functions/stripe-webhook/index.ts`, `supabase/functions/confirm-subscription/index.ts` |
| B10c | Pass content is readable by exactly the athletes who hold it: assigned rows, own coach's rows, and the track nodes of the caller's own active/completed enrolment or attached plan (`my_track_ids`, invoker, authenticated only). Every other workout and diet plan of a coach stays hidden. | `workouts_select`, `diet_plans_select` (migration 20260908120000), child tables parent-scoped | `supabase/security/pass_content.sql` |
| B10d | Push from the database. `trg_notify_push` (definer, not RPC-callable) POSTs notification rows to `send-push-notification` via pg_net with the Vault hook secret in `x-notify-hook` and the public anon key as bearer; the function treats that header like the service role for recipient resolution only (title/body/url come from the row, which only the server writes for request/pass/new_client types: `guard_notification_insert`). Named recipients resolve by id; relationship = coach/athlete or requested coach/athlete. | `supabase/functions/send-push-notification/index.ts`, migration 20260909010000, `guard_notification_insert` | `supabase/security/coach_request.sql` |
| B10e | Athlete inbox. A notification row names one recipient; an athlete reads/marks only rows with their own `client_id`; ordinary roles may write only assignment types, a coach only into their own athletes' inboxes, and every athlete-facing type (`coach_accepted`, `coach_declined`, `session`, `pass`) is written by definer triggers / RPCs. `metadata.url` is validated by the bridge and matched against declared routes on the phone. | `guard_notification_insert`, `notifications_*` policies (20260909020000), `notify_client_on_*` triggers | `supabase/security/client_notifications.sql` |
| B13 | AI and paid compute. Every model call is per-user rate-limited (fails closed) and consent-gated. | `guardRate` buckets (solo-corner 60/h 200/d, solo-program 4/h 10/d, solo-nutrition 3/h 6/d (global 300/d), coach-assistant 60/h 200/d, generate-* 30–40/h, food-image 60/h, text-to-speech 100/h, create-mux-stream 20/h); `AiConsentSheet`. | `supabase/functions/_shared/rateLimit.ts`, `_shared/ai.ts`, `components/solo/AiConsentSheet.tsx`, `lib/aiConsent.ts` |
...
| The corner's read of progress (AI text + the facts it was given) | `client_progress_reads`, `client_checkins.corner_reply` (migration 20260916010000); edge fn `solo-progress` | Written only by `solo-progress` (service role; premium + Solo-only + `guardRate` 6/h 20/d global 2000); athlete selects own rows; proofs `supabase/security/progress_reads.sql` | B1, B5 |
