...
**A9. Account pre-binding (identity).** A coach types `victim@x.com` into a client row (Add athlete → manual) before the victim signs up; the auth.users triggers `handle_new_client_user` / `link_client_auth_user` bind that row to whoever signs up with that email or phone first. With Supabase email confirmation OFF (auto-confirm), an attacker who knows the victim's contact signs up as them and receives the coach's roster slot, workouts and messages meant for the victim; `link_client_to_auth_user` has the same dependency (it trusts the JWT email). **Open**; severity depends on the Auth setting, which cannot be read from here. The invitation path (`accept_invite`) does not have this problem because the code is the credential.
...
1. **A9 — pre-binding by contact.** Stop the auth.users triggers from binding by email/phone when the email/phone is not verified, or drop auto-binding and rely on invitation codes; confirm the Auth email-confirmation setting and enable leaked-password protection (operator).
