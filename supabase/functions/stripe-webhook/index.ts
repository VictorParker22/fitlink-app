...
serve(async (req) => {
  ...
  for (const secret of secrets) {
    try {
      // constructEvent (sync) throws in the Deno build of the SDK:
      // "SubtleCryptoProvider cannot be used in a synchronous context".
      // Every live delivery was rejected with a 400 until this was async
      // (2026-09-08, the first real pass purchase).
