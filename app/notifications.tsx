...
const getIconConfig = (type: NotificationData['type']) => {
  switch (type) {
    case 'message': return { iconBg: '#6B7280', iconName: 'chatbox-ellipses' as const };
    case 'score': return { iconBg: '#F97316', iconName: 'add' as const };
    case 'water': return { iconBg: '#3B82F6', iconName: 'water' as const };
    case 'workout': return { iconBg: '#84CC16', iconName: 'barbell' as const };
    case 'nutrition': return { iconBg: '#A855F7', iconName: 'nutrition' as const };
    case 'file': return { iconBg: '#EF4444', iconName: 'document-text' as const };
    default: return { iconBg: '#6B7280', iconName: 'notifications' as const };
  }
};
...
export default function NotificationsScreen() {
  ...
  const renderNotification = (item: NotificationData) => {
    const { iconBg, iconName } = getIconConfig(item.type);
    ...
    return (
      <TouchableOpacity 
        key={item.id} 
        style={[styles.card, !item.is_read && { borderWidth: 1, borderColor: 'rgba(249,115,22,0.2)', backgroundColor: '#fff7ed' }]}
        activeOpacity={0.8}
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={styles.textContent}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
               ...
               {!item.is_read && <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: '#F97316', marginTop: 2 }} />}
            </View>
